...
function makeTwo() {
	localStorage.setItem('string2', '7,6,5')
...
function makeThree() {
	localStorage.setItem('user3', 'name=John, age=25');
...
function makeFour() {
	localStorage.setItem('boolean4', true)
...
function makeFive() {
	localStorage.setItem('null5', null)
...
function makeSix() {
	...
	localStorage.setItem('objectSix', object);
...
function makeSeven() {
	...
	language: "английский" }
	const object = JSON.stringify(obj);
	localStorage.setItem('objectSeven', object);
	console.log('Объект obj был записан в Local Storage.');
}
...
function makeEight() {
	...
	const object = JSON.stringify(obj);
	localStorage.setItem('objectEight', object);
	console.log( 'Объект obj был записан в Local Storage.');
}
...
function makeNine() {
	...
	localStorage.setItem('arrayNine',array);
...
function makeEleven() {
	...
	const array = JSON.stringify(arr);
	localStorage.setItem('arrayEleven', array);
	console.log('Массив arr был записан в Local Storage.');
}
...
const removeLastElement = () => {
	if (numbers.length > 0) {
		const numbers = JSON.parse(localStorage.getItem('task19'));
		numbers.pop();
...
const showCart = () => {
	...
	resultElement.textContent = cartItems;
};
...
const checkCookie = () => {
	...
	if (username !== '') {
		...
		console.log(`Cookie с именем 'username`)
	}
};
...
const deleteCookie = (name) => {
	...
	console.log(`cookie ${name} удалено`)
};
